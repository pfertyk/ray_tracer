...
class Sun:
    ...
    All light rays from this light source are parallel (there is no center point).
    Light intensity never fades, but obstacles can stop light rays. This light
    source can be used to easily illuminate the whole scene, while still
    producing specular reflections.
    """
    def __init__(self, color=(127, 127, 127), direction=(0, -1, 0)):
        """
        :param color: represents light color (as a tuple of 3 values, range 0 - 255)
        :param direction: light ray direction (might not be normalized)
        """
